...
# Add some default options to db if they don't already exist.  NO Ucase! [add_option() only add if nothing already in db]
 ...
 add_option("pfl_jqfoot_srctext", 'http://' ); #src file/page for footnote
...
#this is to replace file_get_contents when urls are not open.
...
function jqFootnotes_options_subpanel() {
    if ( isset($_POST['info_update']) ) {
        ...
        update_option('pfl_jqfoot_srctext', $_POST['pfl_jqfoot_srctext']);
        ?>
		...
		<table cellpadding="0" cellspacing="0">
            ...
            <form method="post">
						...
						<h3>General Settings</h3>
						 ...
						 <tr valign="top">
										<th scope="row">Footnote Text Source</th>
										<td>
										 <input type="text" name="pfl_jqfoot_srctext" size="50" value="<?php echo get_option('pfl_jqfoot_srctext'); ?>" /> 
											 <br />This is the page that holds a master list of all footnotes.
										</td>
								</tr>
...
# END ADMIN CONSOLE ################################################################
 ...
 // register the js and css scripts.. but not on admin pages
  ...
  function jqFootnotes($data){	
	...
	$pfl_jqfoot_srctext=get_option('pfl_jqfoot_srctext');
...
//	preg_match_all("/\{\{[0-9]*\}\}/", $data, $foots);    
	...
	if(count($foots[0]) != 0){		//there are footnotes to process!
		...
		foreach($foots[0] as $foot){	//finding the footnotes  foreach($array as $value-of-array)
			...
			//look through external file
			$footTextData=file_get_contents("./wp.static/footnotes.src");
			//$footTextData=file_get_contents($pfl_jqfoot_srctext);
			//$footTextData=url_get_contents($pfl_jqfoot_srctext);
			...
			$foot_start = strpos($footTextData, $foot_delim) + strlen($foot_delim);	//now we're looking through 'the_content' for the actual footnote text (not just inline link)
			$foot_end = strpos($footTextData, $foot_delim, $foot_start);			//find out how long footnote is
			// removing from the text since v1.1 ????
			$foots_text[] =  array(substr($footTextData, $foot_start, $foot_end - $foot_start), $foot_num);
